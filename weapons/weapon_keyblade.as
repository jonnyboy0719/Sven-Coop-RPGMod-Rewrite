...
*	@file
*
*	The original Half-Life version of the crowbar, modified to include a variation of Sven Co-op's electric crowbar attack.
*/
...
class weapon_keyblade : ScriptBasePlayerWeaponEntity
{
	...
	void Precache()
	{
		...
		g_Game.PrecacheModel( "models/afterlife/v_sorakeyblade.mdl" );
		g_Game.PrecacheModel( "models/afterlife/w_sorakeyblade.mdl" );
		g_Game.PrecacheModel( "models/afterlife/p_sorakeyblade.mdl" );
		// Precahe this sprite sheet file!
		g_Game.PrecacheGeneric( "sprites/custom_weapons/weapon_keyblade.txt" );
	...
	bool Deploy()
	{
		...
		return self.DefaultDeploy( self.GetV_Model( "models/afterlife/v_sorakeyblade.mdl" ), self.GetP_Model( "models/afterlife/p_sorakeyblade.mdl" ), CROWBAR_DRAW, "crowbar" );
	}
	...
	bool Swing( int fFirst )
	{
		...
		if ( tr.flFraction >= 1.0 )
		{
			if( fFirst != 0 )
			{
				...
				self.m_flNextPrimaryAttack = WeaponTimeBase() + 0.38;
		...
		else
		{
			...
			if ( self.m_flNextPrimaryAttack + 0.78 < WeaponTimeBase() )
			{
			...
			if( pEntity !is null )
			{
				self.m_flNextPrimaryAttack = WeaponTimeBase() + 0.1; //0.25
...
{
	return "weapon_keyblade";
}

void RegisterKeyBlade()
{
	g_CustomEntityFuncs.RegisterCustomEntity( "weapon_keyblade", GetKeyBladeName() );
	g_ItemRegistry.RegisterWeapon( GetKeyBladeName(), "custom_weapons" );
}
